...
list_t *bl_list_new(int64_t nitems, int64_t *items) {
    list_t *list = calloc2(1, sizeof(list_t));
    if (nitems > 0) {
        list->items = calloc2(nitems, sizeof(int64_t));
...
void bl_list_append(list_t *list, int64_t item) {
    list->items = reallocarray2(list->items, list->len+1, sizeof(int64_t));
...
list_t *bl_list_concat(list_t *a, list_t *b) {
    list_t *list = calloc2(1, sizeof(list_t));
    if (a->len > 0 || b->len > 0) {
        list->items = calloc2(a->len + b->len, sizeof(int64_t));
        if (a->len > 0)
...
list_t *bl_list_slice(list_t *list, int64_t first, int64_t next, int64_t last) {
    ...
    list_t *slice = calloc2(1, sizeof(list_t));
    list->len = slice_len;
    list->items = calloc2(slice_len, sizeof(int64_t));
